...
<div id="page_review">
    <form name="myfrm" id="myfrm" method="post" action="update" enctype="multipart/form-data"
            ...
            <div class="insert_page">
                ...
                <div class="page_info">
                    ...
                    <div class="inin_page">
                        ...
                        <c:if test="${vo.rimage1!=null||vo.rimage2!=null||vo.rimage3!=null}">
                            <%-- <div class="review-uploaded-imgs">--%>
                            <%--    <div class="imgs-body">
                                    업로드된 이미지가 없습니다.
                                </div>--%>
                            <%-- </div>--%>
                        ...
                        <c:if test="${vo.rimage1!=null||vo.rimage2!=null||vo.rimage3!=null}">
                            <div class="review-uploaded-imgs">
                                ...
                                <ul class="imgs-body">
                                    <c:if test="${vo.rimage1!=null}">
                                        <li class="rimage1 li-img" data-src=${vo.rimage1}>

                                            <img class="r" src="/display?fileName=${vo.rimage1}"/>
                                            <img class="x" width="30px" src="/resources/icon/close-window-x-icon.png"/>
                                        </li>
                                    </c:if>

                                    <c:if test="${vo.rimage2!=null}">
                                        <li class="rimage2 li-img" data-src=${vo.rimage2}>
                                            <img class="r" src="/display?fileName=${vo.rimage2}"/>
                                            <img class="x" width="30px" src="/resources/icon/close-window-x-icon.png"/>
                                        </li>
                                    </c:if>

                                    <c:if test="${vo.rimage3!=null}">
                                        <li class="rimage3 li-img" data-src=${vo.rimage3}>
                                            <img class="r" src="/display?fileName=${vo.rimage3}"/>
                                            <img class="x" width="30px" src="/resources/icon/close-window-x-icon.png"/>
                                        </li>
                                    </c:if>
                        ...
                        <div id="rtitle summary_container">
                            <div class="textarea_review">
                                    <textarea name="rtitle" id="textarea_2" class="textarea_2" rows="3" cols="95"
                                              placeholder="한 줄 요약을 입력해주세요" maxlength=30>${vo.rtitle}</textarea>
                            </div>
                ...
                <div id="review_btn">
                    ...
                    <div class="box"></div>
                    <button class="review_btn accept">수정하기
                    </button>
                </div>
            </div>
            <!--page_info-->
...
<script>
    let ratingVO = "${vo.star}";
    /*    console.log(ratingVO)*/

    if (ratingVO) {
        ratingVO = parseInt(ratingVO);
        $('.make_star .fa-solid:nth-child(-n+' + ratingVO + ')').css({color: '#F08d28'});
    }
    ...
    const li_img = document.querySelectorAll('.li-img');
    li_img.forEach(item => {
    ...
    function removeItem(e) {
        const files = [];
        const removeItems = [];
        const removeSet = new Array()
        const rimage1 = "${vo.rimage1}";
        const rimage2 = "${vo.rimage2}";
        const rimage3 = "${vo.rimage3}";
        const node = e.currentTarget;
        const img_src = node.dataset.src;
      ...
      */
        ...
        if (!confirm('이미지를 삭제 하실래요?')) return;


        files.filter(item => item !== img_src).forEach(items => {
            removeItems.push(items)
            /* console.log('removeItems array length :' + removeItems.length)*/
        })
        /*    removeItems.forEach(item => {
                console.log('items to keep : ' + item + '\n' + 'count :' + item.length);
            })*/
        /*console.log(node)*/
        /*    console.log(node.dataset.src);*/
        /* alert(img_src)*/
